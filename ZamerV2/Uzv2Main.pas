...
begin
  ...
  QTemp.Open('select value from zini where name=' + Quotedstr('T45FormHeader'));
...
begin
  ...
  try
    ...
    for i := 1 to 3 do
      for j := 1 to 3 do
      begin
        wrepl('st' + inttostr(i) + inttostr(j), FSopr.StringGrid3.Cells[j, i]);
...
begin
  ...
  begin
    ...
    begin
      ...
      Qinsdvig.ParamByName('PNOM').AsFloat := strtofloat(Comma(CombPNom.Text));
      Qinsdvig.ParamByName('PISP').AsFloat := strtofloat(Comma(CombPIsp.Text));
      Qinsdvig.ParamByName('HUMID').AsFloat := strtofloat(EditHumi.Text);
      Qinsdvig.ParamByName('PRESSUR').AsFloat := strtofloat(EditPress.Text);
