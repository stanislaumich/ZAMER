...
// -----------------------------------------------------------------------------------------------------------------------
// ‘ильтры
const int NUM_READ = 10; // количество усреднений дл€ средних арифм. фильтров

// раст€нутое среднее арифметическое
float midArifm2(float newVal) {
	static byte counter = 0; // счЄтчик
	...
	if (counter == NUM_READ) { // достигли кол-ва измерений
		...
		sum = 0; // обнул€ем сумму
		counter = 0; // сброс счЄтчика
	}
	return prevResult;
}
...
float runMiddleArifmOptim(float newVal) {
	...
	average += newVal; // прибавл€ем новое
...
float expRunningAverageAdaptive(float newVal) {
	static float filVal = 0;
	float k;
	// резкость фильтра зависит от модул€ разности значений
