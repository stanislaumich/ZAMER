...
public:
	...
	int modbus_read_holding_registers(uint16_t address, uint16_t amount,
		uint16_t *buffer);
	int modbus_read_input_registers(uint16_t address, uint16_t amount,
		uint16_t *buffer);
	...
	int modbus_write_coils(uint16_t address, uint16_t amount,
		const bool *value);
	int modbus_write_registers(uint16_t address, uint16_t amount,
...
private:
	...
	void modbus_build_request(uint8_t *to_send, uint16_t address,
		int func) const ;

	int modbus_read(uint16_t address, uint16_t amount, int func);
	int modbus_write(uint16_t address, uint16_t amount, int func,
		const uint16_t *value);
...
inline bool modbus::modbus_connect() {
	...
	setsockopt(_socket, SOL_SOCKET, SO_SNDTIMEO, (const char *)&timeout,
		sizeof(timeout));
	setsockopt(_socket, SOL_SOCKET, SO_RCVTIMEO, (const char *)&timeout,
		sizeof(timeout));
	_server.sin_family = AF_INET;
	_server.sin_addr.s_addr = inet_addr(HOST.c_str());
	_server.sin_port = htons(PORT);

	if (!X_ISCONNECTSUCCEED(connect(_socket, (SOCKADDR*)&_server,
		sizeof(_server)))) {
 ...
 */
inline void modbus::modbus_build_request(uint8_t *to_send, uint16_t address,
	int func) const {
 ...
 */
inline int modbus::modbus_write(uint16_t address, uint16_t amount, int func,
	const uint16_t *value) {
	int status = 0;
	uint8_t *to_send;
 ...
 */
inline int modbus::modbus_read_holding_registers(uint16_t address,
	uint16_t amount, uint16_t *buffer) {
	if (_connected) {
 ...
 * @param buffer      Buffer to Store Data Read from Registers
 */
inline int modbus::modbus_read_input_registers(uint16_t address,
	uint16_t amount, uint16_t *buffer) {
	if (_connected) {
 ...
 */
inline int modbus::modbus_read_coils(uint16_t address, uint16_t amount,
	bool *buffer) {
	if (_connected) {
		if (amount > 2040) {
			set_bad_input();
 ...
 */
inline int modbus::modbus_read_input_bits(uint16_t address, uint16_t amount,
	bool *buffer) {
	if (_connected) {
		if (amount > 2040) {
			set_bad_input();
 ...
 */
inline int modbus::modbus_write_register(uint16_t address,
	const uint16_t &value) {
	if (_connected) {
 ...
 */
inline int modbus::modbus_write_coils(uint16_t address, uint16_t amount,
	const bool *value) {
	if (_connected) {
 ...
 */
inline int modbus::modbus_write_registers(uint16_t address, uint16_t amount,
	const uint16_t *value) {
	if (_connected) {
