...
begin
	...
	begin
		...
		QInsall.ParamByName('u12').AsFloat :=
		  SimpleRoundTo(amax[row, i].u1, -4);
		QInsall.ParamByName('u23').AsFloat :=
		  SimpleRoundTo(amax[row, i].u2, -4);
		QInsall.ParamByName('u31').AsFloat :=
		  SimpleRoundTo(amax[row, i].u3, -4);
	...
	QTemp.SQL.Add
	  ...
	  ' and tip=1) and nomer=' + Quotedstr(nomer) + ' and numisp=' +
	  inttostr(row) + ' and tip=1');
	QTemp.Open;
...
begin
	...
	begin
		...
		QInsall.ParamByName('usred').AsFloat  :=
		  SimpleRoundTo(amax[row, i].usred, -4);
		QInsall.ParamByName('u12').AsFloat :=
		  SimpleRoundTo(amin[row, i].u1, -4);
		QInsall.ParamByName('u23').AsFloat :=
		  SimpleRoundTo(amin[row, i].u2, -4);
		QInsall.ParamByName('u31').AsFloat :=
		  SimpleRoundTo(amin[row, i].u3, -4);
		QInsall.ParamByName('torq').AsFloat :=
		  SimpleRoundTo(QTemp.FieldByName('torq').AsFloat, -4);
		QInsall.ParamByName('rot').AsFloat :=
		  SimpleRoundTo(QTemp.FieldByName('rot').AsFloat, -4);
	...
	QTemp.SQL.Add
	  ...
	  + Quotedstr(nomer) + ' and numisp=' + inttostr(row) +
	  ' and tip=2) and nomer=' + Quotedstr(nomer) + ' and numisp=' +
	  inttostr(row) + ' and tip=2');
...
begin
	...
	begin
		Timer2.Enabled := false;
		ShowMessage
		  ('достигнуто максимальное количество замеров, замер остановлен');
		// exit;
	end;
	if curr = 1 then
	begin
