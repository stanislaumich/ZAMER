  ...
  begin
    ...
    begin
      if TComboBox(Components[i]).Tag <> 500 then
        TComboBox(Components[i]).Items.savetofile(Extractfilepath(paramstr(0)) +
...
begin
  ...
  begin
    ...
    begin
      if TComboBox(Components[i]).Tag <> 500 then
        TComboBox(Components[i]).Items.loadfromfile(Extractfilepath(paramstr(0))
          + '\' + TComboBox(Components[i]).name + '.txt');
    end;
  end;
...
begin
  buttonSelected := MessageDlg('Все ли поля условий испытания заполнены верно?',
  ...
  begin
    ...
    try
      ...
      QInsertNewDvig.ParamByName('UNOM').AsInteger  := strtoint(Edit5.Text);
      QInsertNewDvig.ParamByName('UISP').AsInteger  := strtoint(Edit6.Text);
      QInsertNewDvig.ParamByName('PNOM').AsInteger  := strtoint(Edit7.Text);
      QInsertNewDvig.ParamByName('HUMID').AsFloat   := Strtofloat(Edit8.Text);
      QInsertNewDvig.ParamByName('PRESSUR').AsFloat := Strtofloat(Edit9.Text);
...
begin
  ...
  try
    ...
    for i   := 1 to 5 do
      for j := 1 to 4 do
      begin
        wrepl('Kz' + inttostr(i) + inttostr(j), FKZam.StringGrid1.Cells[j, i]);
      end;
    // прочие хар-ки
...
begin
  ...
  begin
    ...
    StatusBar1.Panels[2].Text := KRTCPConnector1.IP + ':' +
      inttostr(KRTCPConnector1.Port) + '@' + inttostr(KRModbusClient1.Addres);
    Edit1.Color      := clGreen;
