                ...
                begin
                        if TComboBox(Components[i]).Tag <> 500 then
                                TComboBox(Components[i])
                                  .Items.savetofile(Extractfilepath(paramstr(0))
...
begin
        ...
        QDelta.ExecSQL;
        QDelta.sql.clear;
        QDelta.sql.Add('insert into zdelta (name,value) values(' +
                ...
                begin
                        if TComboBox(Components[i]).Tag <> 500 then
                                TComboBox(Components[i])
                                  ...
                                  TComboBox(Components[i]).name + '.txt');
                end;
        end;
...
begin
        ...
        begin
                ...
                try
                        ...
                        QInsertNewDvig.ParamByName('TIPDV').Asstring :=
                          ComboBox1.Text;
                        QInsertNewDvig.ParamByName('NOMDV').Asstring :=
                          Edit4.Text;
                        QInsertNewDvig.ParamByName('POLUS').Asstring :=
                          ComboBox2.Text;
                        ...
                        QInsertNewDvig.ParamByName('ENERGO').Asstring :=
                          ComboBox3.Text;
                        QInsertNewDvig.ParamByName('STENDN').Asstring :=
                          ComboBox4.Text;
                        QInsertNewDvig.ParamByName('STENDA').Asstring :=
                          Label24.Caption;
                        QInsertNewDvig.ParamByName('DOP1').Asstring :=
                          Edit11.Text;
                        QInsertNewDvig.ParamByName('ISPOLN').Asstring :=
                          Edit10.Text;
                        QInsertNewDvig.ParamByName('READY').AsInteger := 0;
                        QInsertNewDvig.ParamByName('NOMER').Asstring := Nomer;
                        QInsertNewDvig.ParamByName('fio').Asstring :=
                          ComboBox5.Text;
                        QInsertNewDvig.ParamByName('regim').Asstring :=
                          ComboBox6.Text;
...
begin
        ...
        while not(Qtemp.eof) do
        begin
                Frh.Stringgrid2.Cells[0, tip] :=
                  Qtemp.FieldByName('pisp').Asstring;
                ...
                Frh.Stringgrid2.Cells[7, tip] :=
                  Qtemp.FieldByName('dpmax').Asstring;
                Qtemp.Next;
                tip := tip + 1;
        end;
        ...
        while not(Qtemp.eof) do
        begin
                FMehan.StringGrid7.Cells[1, Qtemp.FieldByName('numisp')
                  .AsInteger] := Qtemp.FieldByName('u').Asstring;
                FMehan.StringGrid7.Cells[2, Qtemp.FieldByName('numisp')
                  .AsInteger] := Qtemp.FieldByName('torq').Asstring;
                FMehan.StringGrid7.Cells[3, Qtemp.FieldByName('numisp')
                  .AsInteger] := Qtemp.FieldByName('rot').Asstring;
        ...
        while not(Qtemp.eof) do
        begin
                FMehan.StringGrid8.Cells[1, Qtemp.FieldByName('numisp')
                  .AsInteger] := Qtemp.FieldByName('u').Asstring;
                FMehan.StringGrid8.Cells[2, Qtemp.FieldByName('numisp')
                  .AsInteger] := Qtemp.FieldByName('torq').Asstring;
                FMehan.StringGrid8.Cells[3, Qtemp.FieldByName('numisp')
                  .AsInteger] := Qtemp.FieldByName('rot').Asstring;
                if Qtemp.FieldByName('checked').AsInteger = 1 then
                begin
                        case Qtemp.FieldByName('numisp').AsInteger of
        ...
        begin
                ...
                Fnagrev.StringGrid1.Cells
                  [7, Qtemp.FieldByName('tip').AsInteger] := '0';
                Qtemp.Next;
        end;
...
begin
        ...
        try
                ...
                for i := 1 to 10 do
                begin
                        wrepl('u' + inttostr(i) + 'hh',
                          Fhhod.Stringgrid2.Cells[1, i]);
                end;
                for i := 1 to 10 do
                begin
                        wrepl('i' + inttostr(i) + 'hh',
                          Fhhod.Stringgrid2.Cells[2, i]);
                end;
                for i := 1 to 10 do
                begin
                        wrepl('p' + inttostr(i) + 'hh',
                          Fhhod.Stringgrid2.Cells[3, i]);
                end;
                for i := 1 to 10 do
                begin
                        wrepl('R' + inttostr(i) + 'hh',
                          Fhhod.Stringgrid2.Cells[5, i]);
                end;
                ...
                begin
                        wrepl('i' + inttostr(i) + 'rh',
                          Frh.Stringgrid2.Cells[2, i])
                end;
                for i := 1 to 10 do
                begin
                        wrepl('p' + inttostr(i) + 'rh',
                          Frh.Stringgrid2.Cells[3, i]);
                end;
                for i := 1 to 10 do
                begin
                        wrepl('rot' + inttostr(i) + 'rh',
                          Frh.Stringgrid2.Cells[4, i]);
                end;
                for i := 1 to 10 do
                begin
                        wrepl('torq' + inttostr(i) + 'rh',
                          Frh.Stringgrid2.Cells[5, i]);
                end;
                // сопротивление
                ...
                begin
                        wrepl('Pr' + inttostr(j + 100),
                          Fproch.StringGrid1.Cells[j, 1]);
                end;
                for j := 1 to 18 do
                begin
                        wrepl('Pz' + inttostr(j + 100),
                          Fproch.StringGrid1.Cells[j, 2]);
                end;
                for i         := 1 to 2 do
                        for j := 1 to 8 do
                        begin
                ...
                WordApp.ActiveDocument.SaveAs(ReportPath + '\' + Nomer
                  + '.docx');
...
begin
        /// ////
        QUpdDvig.Close;
        QUpdDvig.ParamByName('DATA').Asstring :=
          DateToStr(DateTimePicker1.Date);
...
begin
        ...
        begin
                ...
                Qtemp.sql.clear;
                Qtemp.sql.Add('insert into ini(name, value, tag) values (');
                Qtemp.sql.Add(Quotedstr('TIPDVIG') + ', ' +
                  Quotedstr(ComboBox1.Items[i]) + ', 0)');
                Qtemp.ExecSQL;
        end;
