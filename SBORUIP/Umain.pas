...
var
    ...
    // *********/ скользящее среднее
    Maxsred: Integer;
    Nsred: Integer; // счетчик
    Asred: array [1 .. 50] of real; // сам массив
    ...
    INVERT: Integer;
    press: Integer;
    koef_to_auto: Integer;
    // это переменная которая переключает кнопки увеличения и уменьшения
    // если установлена в 1 то нажатие кнопки выкл реле, если в 0 - то вкл
    // одной кнопкой можно переключать реле
...
begin
    if INVERT = 0 then
        ...
        Com.WriteStr('1');
    FSett.Memo1.Lines.Add('вкл БОЛЬШЕ ');
    BitBtn4.Caption := '<БОЛЬШЕ>';
    BitBtn4.Font.Color := clRed;
    BitBtn5.Caption := 'МЕНЬШЕ';
...
begin
    if INVERT = 0 then
        ...
        Com.WriteStr('2');
    // Com.WriteStr('1');
    FSett.Memo1.Lines.Add('вкл МЕНЬШЕ ');
    BitBtn5.Caption := '<МЕНЬШЕ>';
...
begin
    Com.WriteStr('0');
    FSett.Memo1.Lines.Add('вкл СТОП ');
    BitBtn5.Caption := 'МЕНЬШЕ';
    BitBtn5.Font.Color := clBlue;
    BitBtn4.Caption := 'БОЛЬШЕ';
    BitBtn4.Font.Color := clBlue;
...
begin
    // *********/ скользящее среднее
    // Maxsred:integer // колво элементов
    // Nsred:integer;// счетчик
    // Asred:array[1..Maxsred] of real;// сам массив
    // Rsred:real;// вычисленное среднее dU
    // *********/
    Nsred := Nsred + 1;
    if Nsred > Maxsred then
        Nsred := 1;
    Asred[Nsred] := s;
    res := 0;
    for I := 1 to Maxsred do
        res := res + Asred[I];
    res := res / Maxsred;
    getsred := res;
end;
...
begin
    ...
    if dU < strtofloat(Edit2.Text) then
        initsred(dU);
    ...
    if abs(dU) < strtofloat(S3) then
        koef_to_auto := 1; // если попали в нормальное отклонение отключаем коэффициент
    if abs(dU) > strtofloat(S3) * koef_to_auto then
    begin // напряжение упало более чем на S3
        // отменяем все действия пока не попадем в диапазон S3
        // koef_to_auto - 1 для обычной работы и 100 если надо подогнать по нажатии 6 кнопок
        //log('S3 за пределами');
        //log(floattostr(strtofloat(S3) * koef_to_auto));
        Exit; // всё бросили и вышли
    end
    else
    begin // всё нормально, запоминаем и работаем
        pU := cU;
    end;
